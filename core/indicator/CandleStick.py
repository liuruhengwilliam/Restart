...
def show_candlestick(target,dfData, ma, BBands, periodName,path=None):
    """ 内部接口API:
    ...
    try:
        ...
        if path is None:
            ...
            plt.savefig('%s%s-%s-%s.png'%(path,target,periodName,timestamp),dpi=200)
...
def manual_show_candlestick(target,periodName,data,path):
    """ 外部接口API: 手动绘图使用。
    ...
    show_candlestick(target,dataPicked,ma,bbands,periodName,path)
